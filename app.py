...
# Model Path
MODEL_PATH = "vitamin_model.keras" 
...
def generate_pdf(prediction_class, confidence, symptoms, remedy_text):
    try:
        # Convert remedy markdown to simple HTML for the PDF
        remedy_html = markdown.markdown(remedy_text)
        
        doc = fitz.open()
        page = doc.new_page()
        
        # Modern CSS for a clinical report
        css = """
        <style>
            body { font-family: 'Helvetica', sans-serif; color: #333; line-height: 1.6; }
            .header { background-color: #007bff; color: white; padding: 20px; text-align: center; border-radius: 5px; }
            .section { margin-top: 20px; padding: 15px; border-left: 5px solid #007bff; background-color: #f8f9fa; }
            .prediction { color: #28a745; font-weight: bold; font-size: 18px; }
            .remedies { background-color: white; border: 1px solid #ddd; padding: 15px; border-radius: 5px; }
        ...
        {css}
        <div class="header">
            <h1>🩺 Vitamin Deficiency Diagnostic Report</h1>
            <p>Multimodal AI Healthcare Analysis</p>
        </div>
        
        <div class="section">
            <h2>📊 Analysis Summary</h2>
            <p><strong>Top Prediction:</strong> <span class="prediction">{prediction_class}</span></p>
            <p><strong>Confidence:</strong> {confidence:.2f}%</p>
        </div>
        
        <div class="section">
            <h2>✍️ Symptoms Reported</h2>
            <p>{symptoms}</p>
        </div>
        
        <div class="section">
            <h2>💡 AI Recommended Remedies & Advice</h2>
            <div class="remedies">
                {remedy_html}
            </div>
        </div>
        ...
        <div class="footer">
            <p>Generated by Vitamin Deficiency Detector AI | {os.getenv('STREAMLIT_APP_URL', 'Local Instance')}</p>
            <p>Disclaimer: This is an AI-generated assessment for educational purposes and is NOT medical advice. Please consult a qualified doctor.</p>
        </div>
        """
        
        # Insert the styled HTML into the page
        page.insert_htmlbox(page.rect, report_html)
